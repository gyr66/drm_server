...
async function encryptVideo(videoInputPath, videoOutputPath, contentId) {
  ...
  let res = await axios.post(keyServerUrl, keyServerRequest, { headers: { 'Content-Type': 'application/json' } });
  ...
  let key = Buffer.from(contentKeyResponse.tracks[0].key, "base64").toString("hex");
  let keyId = uuid.unparse(Buffer.from(contentKeyResponse.tracks[0].key_id, "base64")).replace(/-/g, '');
  logger.debug("key: " + key);
  logger.debug("keyId: " + keyId);
  return new Promise(resolve => {
    exec(`./packager 'in=${videoInputPath},stream=video,output=${videoOutputPath}/video.mp4,init_segment=${videoOutputPath}/video/init.mp4,segment_template=${videoOutputPath}/video/$Number$.m4s' \
    ...
    --clear_lead=0`, (err, stdout, stderr) => {
